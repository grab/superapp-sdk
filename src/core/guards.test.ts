...
describe('guards', () => {
  describe('isSuccess', () => {
    it('should return true for status code 200', () => {
      const response: SDKResponse = { status_code: 200, result: 'data' };
      expect(isSuccess(response)).toBe(true);
    });

    it('should return true for status code 204', () => {
      const response: SDKResponse = { status_code: 204 };
      expect(isSuccess(response)).toBe(true);
    });

    it('should return false for status code 299', () => {
      const response: SDKResponse = { status_code: 299, result: 'data' };
      expect(isSuccess(response)).toBe(false);
    });

    it('should return false for status code 199', () => {
      const response: SDKResponse = { status_code: 199, result: 'data' };
      expect(isSuccess(response)).toBe(false);
    });

    it('should return false for status code 300', () => {
      const response: SDKResponse = { status_code: 300 };
      expect(isSuccess(response)).toBe(false);
    });

    it('should return false for status code 400', () => {
      const response: SDKResponse = { status_code: 400, error: 'Bad Request' };
      expect(isSuccess(response)).toBe(false);
    });

    it('should return false for status code 500', () => {
      const response: SDKResponse = { status_code: 500, error: 'Server Error' };
      expect(isSuccess(response)).toBe(false);
    });
  });
  ...
  describe('isOk', () => {
    ...
    it('should return false for status code 201', () => {
      const response: SDKResponse = { status_code: 201, result: 'data' };
  ...
  describe('isNoContent', () => {
    ...
    it('should return false for status code 201', () => {
      const response: SDKResponse = { status_code: 201 };
      expect(isNoContent(response)).toBe(false);
    });
  });
  ...
  describe('isRedirection', () => {
    it('should return true for status code 302', () => {
      const response: SDKResponse = { status_code: 302 };
    ...
    it('should return false for status code 301', () => {
      const response: SDKResponse = { status_code: 301 };
      expect(isRedirection(response)).toBe(false);
    });
  });
  ...
  describe('isClientError', () => {
    ...
    it('should return true for status code 401', () => {
      const response: SDKResponse = { status_code: 401, error: 'Unauthorized' };
      expect(isClientError(response)).toBe(true);
    });

    it('should return true for status code 403', () => {
      const response: SDKResponse = { status_code: 403, error: 'Forbidden' };
      expect(isClientError(response)).toBe(true);
    });

    it('should return true for status code 404', () => {
      const response: SDKResponse = { status_code: 404, error: 'Not Found' };
      expect(isClientError(response)).toBe(true);
    });

    it('should return true for status code 424', () => {
      const response: SDKResponse = { status_code: 424, error: 'Failed Dependency' };
      expect(isClientError(response)).toBe(true);
    });

    it('should return true for status code 426', () => {
      const response: SDKResponse = { status_code: 426, error: 'Upgrade Required' };
      expect(isClientError(response)).toBe(true);
    });
    ...
    it('should return false for status code 399', () => {
      const response: SDKResponse = { status_code: 399 };
      expect(isClientError(response)).toBe(false);
    });
  ...
  describe('isServerError', () => {
    it('should return true for status code 500', () => {
      const response: SDKResponse = { status_code: 500, error: 'Internal Server Error' };
      expect(isServerError(response)).toBe(true);
    });

    it('should return true for status code 501', () => {
      const response: SDKResponse = { status_code: 501, error: 'Not Implemented' };
      expect(isServerError(response)).toBe(true);
    });
    ...
    it('should return false for status code 600', () => {
      const response: SDKResponse = { status_code: 600 };
      expect(isServerError(response)).toBe(false);
    });
  ...
  describe('isError', () => {
    ...
    it('should return false for status code 503', () => {
      const response: SDKResponse = { status_code: 503, error: 'Service Unavailable' };
      expect(isError(response)).toBe(false);
    });

    it('should return false when error string is present with non-error status code', () => {
      const response: SDKResponse = { status_code: 200, error: 'Some error' };
      expect(isError(response)).toBe(false);
    });
    ...
    it('should return true for 4xx status code even with empty error string', () => {
      const response: SDKResponse = { status_code: 400, error: '' };
      expect(isError(response)).toBe(true);
    });

    it('should return false when error is empty string and status is success', () => {
      const response: SDKResponse = { status_code: 200, error: '' };
      expect(isError(response)).toBe(false);
    });
  });
  ...
  describe('hasResult', () => {
    ...
    it('should return true when result is an object', () => {
      const response: SDKResponse = { status_code: 200, result: { id: 1 } };
      expect(hasResult(response)).toBe(true);
    });

    it('should return true when result is a number', () => {
      const response: SDKResponse = { status_code: 200, result: 42 };
      expect(hasResult(response)).toBe(true);
    });

    it('should return true when result is an empty string', () => {
      const response: SDKResponse = { status_code: 200, result: '' };
      expect(hasResult(response)).toBe(true);
    });
    ...
    it('should return true when result is false', () => {
      const response: SDKResponse = { status_code: 200, result: false };
      expect(hasResult(response)).toBe(true);
    });

    it('should return true when result is 0', () => {
      const response: SDKResponse = { status_code: 200, result: 0 };
      expect(hasResult(response)).toBe(true);
    });

    it('should return false when result is null', () => {
      const response: SDKResponse = { status_code: 200, result: null };
      expect(hasResult(response)).toBe(false);
    });

    it('should return false when result is undefined', () => {
      const response: SDKResponse = { status_code: 200, result: undefined };
      expect(hasResult(response)).toBe(false);
    });
  ...
  describe('guard consistency', () => {
    it('isError should match isClientError || isServerError', () => {
      const responses: SDKResponse[] = [
        ...
        { status_code: 424, error: 'failed dependency' },
        { status_code: 426, error: 'upgrade required' },
        { status_code: 500, error: 'internal error' },
        { status_code: 501, error: 'not implemented' },
        { status_code: 418, error: "i'm a teapot" },
        { status_code: 503, error: 'service unavailable' },
        { status_code: 599, error: 'server error' },
      ];

      for (const response of responses) {
        expect(isError(response)).toBe(isClientError(response) || isServerError(response));
      }
    });
  });
});
